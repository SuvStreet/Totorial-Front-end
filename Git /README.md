...
<h2 name='отправка_изменений_с_компьютера_на_удаленный_репозиторий'>Отправка изменений с компьютера на удаленный репозиторий</h2>
  ...
  -  `git push --force` - принуждает удалённый репозиторий принять ваши локальные изменения, даже если они конфликтуют с тем, что уже есть в удалённом репозитории.
  > стоит быть осторожным с этой командой, так как она может привести к потере данных, если использована неправильно.
...
<h2 name='основные_команды_git'>Основные команды Git</h2>
  ...
  - `git reset HEAD~` - удалить последний коммит. ***HEAD — это указатель на текущую ветку, которая, в свою очередь, является указателем на последний коммит, сделанный в этой ветке.  Как правило, самое простое считать `HEAD снимком вашего последнего коммита`.***
  
  - `git reset --hard HEAD~` - отменяет все изменения в коде разработки возвращая файл к commit-у в котором ещё не сделаны изменения.
  
  - `git reset --hard [commit_hash_code]` - отменяем коммит по его хеш-коду ***[commit_hash_code] заменить на хэш-код коммита, к которому хотим вернуться***
  
  - `git revert [commit_hash_code]` - создаёт новый коммит, который вносит изменения, противоположные указанному коммиту, по существу отменяя его. ***[commit_hash_code] заменить на хэш-код коммита, к которому хотим вернуться***
